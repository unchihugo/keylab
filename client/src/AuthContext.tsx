...
interface AuthContextProps {
    ...
    register: (forename: string, surname: string, email: string, password: string) => Promise<void>;
    logout: () => void;
}
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const register = async (forename: string, surname: string, email: string, password: string) => {
        try {
            if (!isTesting) {
                const data = await authService.register(forename, surname, email, password);
                console.log(data.message);
            }
            setIsAuthenticated(true);
        } catch (error) {
